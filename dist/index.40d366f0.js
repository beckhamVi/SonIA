...
function toggleTheme() {
    ...
    if (isDarkTheme) themeIcon.classList.replace('fa-sun', 'fa-moon');
...
function checkElements() {
    ...
    for (const [name, element] of Object.entries(elements))if (!element) {
...
async function getAccessToken() {
    try {
        ...
        if (!response.ok) throw new Error("Error en la autenticaci\xf3n");
        const data = await response.json();
        return data.access_token;
    } catch (error) {
        console.error("Error obteniendo token:", error);
        alert("Error de autenticaci\xf3n con Spotify");
        return null;
    }
}
async function searchSong(song) {
    try {
        ...
        const response = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(song)}&type=track&market=ES`, {
            method: "GET",
            headers: {
...
async function displayRecommendations(songURI) {
    try {
        ...
        const response = await fetch(`https://api.spotify.com/v1/recommendations?seed_tracks=${trackID}&limit=20&market=ES`, {
            method: "GET",
            headers: {
                Authorization: "Bearer " + accessToken
            }
...
function playNextTrack() {
    ...
    if (isShuffleOn) currentTrackIndex = Math.floor(Math.random() * recommendations.length);
    else currentTrackIndex = (currentTrackIndex + 1) % recommendations.length;
    loadTrackInPlayer(recommendations[currentTrackIndex]);
}
function playPrevTrack() {
    if (recommendations.length === 0) return;
    if (isShuffleOn) currentTrackIndex = Math.floor(Math.random() * recommendations.length);
    else currentTrackIndex = (currentTrackIndex - 1 + recommendations.length) % recommendations.length;
    loadTrackInPlayer(recommendations[currentTrackIndex]);
}
// Event Listeners
...
document.addEventListener('click', (e)=>{
    if (!sidebar.contains(e.target) && !menuToggle.contains(e.target) && sidebar.classList.contains('active')) toggleSidebar();
});
...
searchButton.addEventListener("click", async ()=>{
    ...
    if (!songName) {
        alert("Por favor ingresa el nombre de una canci\xf3n");
        return;
    }
    ...
    } else songInfo.innerHTML = "Canci\xf3n no encontrada.";
});
// Controles de reproducción
nextButton.addEventListener("click", playNextTrack);
prevButton.addEventListener("click", playPrevTrack);

//# sourceMappingURL=index.40d366f0.js.map
